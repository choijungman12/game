...
- **무료 Firebase 연동 (앱에서 바로)**: 관리자 → **연동 설정**에서 Realtime Database URL을 붙여넣고 ‘연결 테스트 → 저장’ 하면 여러 휴대폰이 같은 게임에 접속합니다. 코드 수정이 필요 없습니다.
- **라이브 공유 게임**: QR로 접속한 모든 참가자가 **같은 게임**을 하며, 진행(시작·정답 공개·다음 문제)은 **관리자 페이지**에서 제어합니다.
- **팀전 지역명 편집**: 관리자 → **팀전 관리 → 보드 칸(지역) 편집**에서 브루마블 땅 이름·가격을 수정·저장하면 보드에 즉시 반영됩니다.
- **회사소개·물건지도 카테고리**: 개인전 문제에 대연P&C 회사 소개와 주요 사업지역(주문진·속초·구기동·당진) 문항이 추가되었습니다. 관리자 문제 관리에서 실제 매물 정보로 수정할 수 있습니다.
